...
############# To check ##############################
#####################################################
def digi_image2(file):
    data = open(file,'r')
    data.readline()
    x,y = [],[]
    for line in data:
        P = line.split(',')
        x.append(round(float(P[0]),2))
        y.append(round(float(P[1].strip('\n')),2))
    return x,y
...
x_cw,y_cw   = digi_image('cw.csv')
x_ccw,y_ccw = digi_image2('ccw.csv')    
    

ip1 = [1.179,0,0,0,0,0,0]
ip2 = [1.179,0,0,0,0,0,0]
# ip1 = [1.29,0,0,0,0,0,0]
# ip2 = [1.29,0,0,0,0,0,0]
...
for i in range(N):
    if i < 0:
        temp = activate(data[-1],0.61,12.034) #-0.61
    else:
        temp = activate(data[-1],0.61,12.034)
    data.append(temp)
    x1.append(temp[3]/7)
    y1.append(temp[4]/7)
...
for j in range(N):
    if j < 600:
        temp1 = activate(data1[-1],-0.61,12.034) #-0.61
    else:
        temp1 = activate(data1[-1],-0.61,12.034)
    data1.append(temp1)
    x2.append(temp1[3]/7)
    y2.append(temp1[4]/7)
...
plt.plot(y1,x1,'green',label = " Designed Model ")
plt.plot(y2,x2,'green')
plt.plot(x_cw,y_cw,'m--',label=" Yasukawa Model")
plt.plot(x_ccw,y_ccw,'m--')
...
plt.title("simulation for 120 seconds for n= 12 rps with u_0 = 1.179 m/s ")
# plt.title("Equation 16")
plt.legend(loc="best")
plt.grid()
plt.show()
######################################################
