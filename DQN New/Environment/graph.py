...
def plot_result1(NoEpi, Cumulative_reward,path,N="End"):
    ...
    plt.grid()
    plt.legend(loc="best")
    plt.savefig(os.path.join(path,"pic1_epi_"+str(N)+".jpg" ),dpi=1200)
    plt.close()
...
def plot_result2(HEs,MSE,path,N="End"):
    ...
    plt.plot(MSE,color="b",label = "Cumulative Heading Error",alpha=0.2)
    plt.plot(running_avg2,color='b',label="Running Average" )
    plt.xlabel("No of Episode")
    plt.ylabel("Heading Error in degree")
    plt.legend(loc="best")
    plt.grid()
    plt.savefig(os.path.join(path,"pic2_epi_"+str(N)+".jpg" ),dpi=1200)
    plt.close()#plt.show()
