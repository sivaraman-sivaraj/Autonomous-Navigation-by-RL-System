...
class MMG():
    ...
    def MMG_Time_Derivative(u,v,r,x,y,psi,delta,rps = 1.783):
        """
        ----------
    ...
    def activate(ip,delta,rps = 1.783):
...
class LOS():
    ...
    def get_y_e_HE(ip,wp_k,wp_k_1):
        ...
        if ip[5] >= 0:
            psi_actual     = ip[5]%np.pi
            HE             = psi_d - psi_actual               # Heading error
        elif ip[5] < 0:
            psi_actual     = ip[5] % (-np.pi)
            HE             = psi_d - psi_actual
    ...
    def activate(ip,wpA,H):
        ...
        #############################################
        y_e, HE      =  LOS.get_y_e_HE(ip, wp_k, wp_k_1)
        #############################################
        ########## Updating  the Memory #############
        #############################################
        if wp_near == len(SP)-1:
...
class MLFFNN(nn.Module):
    ...
    def forward(self, x):
        x = torch.tanh(self.ipL(x))#sigmiodal activation function
...
class Surrounding(): #basinGrid-s0
     ...
     def step(self,C):
         ...
         if self.Flag:
             ...
             if wp_Analysis.Quadrant_position([self.op[3],self.op[4]]) == self.St_Q:
         ...
         if not self.Flag:
             ...
             if self.St_angle >= 0:
                 if 0 <= (self.St_angle - self.psi_actual) <= (np.pi/12):
                     self.H[2]  = True
             if self.St_angle < 0 :
                 if 0 > (self.St_angle - self.psi_actual) >= -(np.pi/12):
                     self.H[2]  = True
...
plt.figure(figsize=(9,6))
D = len(NoEpi)
running_avg2 = np.empty(D)
for t in range(N):
        running_avg2[t] = np.mean(NoEpi[max(0, t-100):(t+1)])
plt.plot(running_avg2,color='r')
plt.title("Episode Duration Plot for Training the Model")
plt.xlabel("No of Episode")
...
print("Total time taken for 15000 episode", (end - start),'seconds')
