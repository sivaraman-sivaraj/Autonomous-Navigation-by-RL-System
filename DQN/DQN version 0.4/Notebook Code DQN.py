...
class LOS():
    ...
    def activate(ip,wpA,H):
        ...
        #############################################
        y_e, HE,g_p      =  LOS.get_y_e_HE(ip, wp_k, wp_k_1)
        #############################################
        ########## Updating  the Memory #############
        #############################################
        if End_flag == False:
...
class Reward():
    def get(ip,op,y_e,HE,G):
        ...
        x_d1 = np.square(G[0] - ip[3])
        y_d1 = np.square(G[1] - ip[4])
...
def eps_calculation(i_episode):
    ...
    eps = end + (start- end) * math.exp(-1 * i_episode / 400)
    return eps
...
def optimize_model():
    ...
    #loss = F.F1(state_action_values, expected_state_action_values)
    # Optimize the model
    optimizer.zero_grad()
    loss.backward()
...
def plot_result2(HEs,MSE):
    ...
    plt.title("DQN Training Resuts : F1 Smooth Loss , Heading Error ")
    plt.xlabel("No of Episode")
    plt.ylabel("MSE Loss")
    # plt.ylim(0,110)
    plt.legend(loc="best")
