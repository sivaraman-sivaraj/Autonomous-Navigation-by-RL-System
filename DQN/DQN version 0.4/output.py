...
D = {'0':-35,'1':-20,'2':-5,'3':0,'4':5,'5':20,'6':35}
...
w1 = np.load("layer1 weight.npy")
w2 = np.load("layer2 weight.npy")
w3 = np.load("layer3 weight.npy")
w4 = np.load("layer4 weight.npy")
b1 = np.load("layer1 bias.npy")
b2 = np.load("layer2 bias.npy")
b3 = np.load("layer3 bias.npy")
b4 = np.load("layer4 bias.npy")
trained_net.ipL.weight.data= torch.tensor(w1)
trained_net.HL1.weight.data= torch.tensor(w2)
trained_net.HL2.weight.data= torch.tensor(w3)
trained_net.opL.weight.data= torch.tensor(w4)
trained_net.ipL.bias.data= torch.tensor(b1)
trained_net.HL1.bias.data= torch.tensor(b2)
trained_net.HL2.bias.data= torch.tensor(b3)
trained_net.opL.bias.data= torch.tensor(b4)

ip      = torch.tensor([1.179,0,0,0,0,0])


x,y = [],[]

for i in range(350):
    temp = torch.tensor(ip)
    ip = temp.clone().detach()
    opt   = trained_net(ip)
    delta = np.argmax(opt.detach().numpy())
    op    = MMG.activate(ip,D[str(delta)]) 
    x.append(op[3])
    y.append(op[4])
    ip = op
    
plt.figure(figsize=(9,6))
plt.plot(x,y,'g',label = "Trained Path")
plt.plot([0,300],[0,300],color="b")
plt.title("Trained Trajectory")
plt.grid()
plt.plot([0,0],[300,300],'r--',label="Target Path")
plt.xlabel("Advance")
plt.ylabel("Transfer")
plt.legend(loc="best")
plt.show()
