...
class LOS():
    ...
    def get_y_e_HE(ip,wp_k,wp_k_1):
        ...
        psi_d          = (np.pi/2) - g_p + np.arctan(-y_e/delta_h) - beta # Desired Heading angle # equation 29
    ...
    def activate(ip,wpA,H):
        ...
        End_flag = False                           # make ensure that the last waypoint
...
class CTE():
    ...
    def conversion_factor(x,mu,sigma):
        y_max = CTE.normal_df(x,mu,sigma)
        factor = 100/y_max
        return factor
...
class MLFFNN(nn.Module):
    def __init__(self):
        ...
        self.opL = nn.Linear(96,23)  # output layer
        
    def forward(self, x):
        x = torch.relu(self.ipL(x))#sigmiodal activation function
...
class Surrounding(): #basinGrid-s0
     ...
     def __init__(self,wpA,grid_size,u,prp):
        ...
        self.actions_set        = {'0':-35,'1':-20,'2':-10,'3':-5,'4':0,'5':5,'6':10,'7':20,
                                   '8':35}
     ...
     def step(self,C):
         ...
         if self.Flag:
             ...
             self.reward_a   = CTE.get(self.y_e,self.HE)
             #################################
             ###### Next State Update ########
             #################################
             ...
             if int(self.int_x) == int(self.goals[0]) and int(self.int_y) == int(self.goals[1]) :
                self.done = True
             #### @2 by unwanted loop ####
             if self.current_state[5] > 6.28:
                 self.done = True
             #### @3 by Heading Error ####
             if abs(self.HE) > (np.pi*2/3):
                 self.done = True
             
             #### @4 by Cross Track error ####
             if abs(self.y_e) >= 35:
                 self.done = True
...
def eps_calculation():
    ...
    eps = end + (start- end) * math.exp(-1 * steps_done / 500)
...
def plot_result2(HEs,MSE):
    ...
    plt.title("DQN Training Resuts : Mean Square Loss , Heading Error ")
    plt.xlabel("No of Episode")
...
###################################################
env                     = Surrounding(wpA, GFL, initial_velocity, prp)
