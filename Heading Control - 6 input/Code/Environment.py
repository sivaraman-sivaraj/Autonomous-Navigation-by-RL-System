...
class load(Env): # NavigationRL-v0
     ...
     def __init__(self,wp,wpA,u):
        ...
        #### Goal points, Steps, NoEpi #####
        self.goals          = self.wpA[2][0]
        self.steps          = self.wpA[2][1]
        self.NoEpi          = self.wpA[2][2]
        #### R_1 ####
        self.R_1            = wpA[3]
        #### initial conditions ####
        self.done,self.viewer   = False, None           # see here
        self.st_x, self.st_y    = 0,0
        self.actions_set        = {'0':-35,'1':-15,'2':0,'3':15,'4':35}
     ...
     def reset(self):
        #######################
        ## H = [last quadrant, last waypoint,previous heading error]
        ########################
        self.H                                = [0,0,-self.St_angle]
        self.done                             = False
        self.current_state                    = torch.tensor([self.u,0,0,self.st_x, self.st_y,0])
        return self.current_state,self.H
    
     def step(self,C):
         self.action,self.H,self.step_i       = C[0],C[1],C[2]
         self.goal                            = C[3]
         self.HE_old                          = self.H[2] 
         ...
         self.reward_a                        = Reward.get(self.ip,self.op,self.y_e,self.HE,self.HE_old,
                                                           self.goal,self.R_1,self.step_i )
         # print(self.reward_a)
