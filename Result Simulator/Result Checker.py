...
rps                   = (scaled_u*9.86*1.783)/(7.75*0.216) # Froude Scaling
# waves                 = False                              # for calm water, set as False
waves                 = True
###############################################
############ waypoints maker ##################
###############################################
# wp,x_path,y_path,L       = waypoints.straight_line(250,135)
wp,x_path,y_path,L       = waypoints.spiral(25)
# wp,x_path,y_path,L       = waypoints.Fibbanaci_Trajectory(10)
# wp,x_path,y_path,L       = waypoints.cardioid(30) 
# wp,x_path,y_path,L       = waypoints.parametric(30)
# wp,x_path,y_path,L       = waypoints.Arc_spline()
# wp,x_path,y_path,L       = waypoints.Ellipse(50,20)
...
net                     = Q_network.FFNN()
net.load_state_dict(torch.load("W4000.pt"))
...
# Create_GIF(frames,filename='ellipse.gif')
####################################
###### Result Plotting #############
####################################
plt.figure(figsize=(9,6))
# plt.plot(X,Y,color='green',label="DQN Trained Path in Calm Water")
...
plt.title("Navigation by DQN $\longrightarrow$ Spiral Curve Tracking in Waves ")
# plt.title("In Calm Water")
plt.legend(loc="best")
