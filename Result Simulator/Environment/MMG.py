...
def los_mmg_normalizer(theta):
    ...
    Parameters
    ----------
    theta : based on ship mathematical model

    Returns
    -------
    theta_new : heading angle in LOS frame
...
def MMG_Time_Derivative(u,v,r,x,y,psi,delta,rps,w_flag):
    """
    ----------
    ...
    uR    = 1*epsilon * uP * np.sqrt( (eta*uR2) + (1-eta))
    ...
    f_alpha     = (6.13*AR/(AR+2.25))+12       # Rudder lift gradient coefficient 2.747
    ...
    xR         = -0.57 #-0.5 * Lpp          # Longitudinal coordinate of rudder position 
    ...
    Amplitude          = 3.2
    wave_frequency     = 0.4390*(Lamda)**0.5
    heading_angle      = np.pi
    ...
    hangle_desire  = heading_angle - los_mmg_normalizer(psi) #wrap2pi(heading_angle - wrap2pi(psi)) #  #yaw angle
...
def activate(ip,delta,rps,w_flag):
    ...
    ###############################
      ...
      K2_ = MMG_Time_Derivative(u+(K1[0]/2), v+(K1[1]/2), r+(K1[2]/2), x+(k1[0]/2), y+(k1[1]/2), psi+(k1[2]/2),delta,rps,w_flag)
      K2  = h*K2_[:3]
      k2  = h*(K0[:3] + 0.5*K1)
          
      K3_ = MMG_Time_Derivative(u+(K2[0]/2), v+(K2[1]/2), r+(K2[2]/2),x+(k2[0]/2), y+(k2[1]/2), psi+(k2[2]/2),delta,rps,w_flag)
      K3  = h*K3_[:3]
      k3  = h*(K0[:3] + 0.5*K2)
     
      K4_ = MMG_Time_Derivative(u+(K3[0]), v+(K3[1]), r+(K3[2]), x+(k3[0]), y+(k3[1]), psi+(k3[2]),delta,rps,w_flag)
      K4  = h*K4_[:3]
      k4  = h*(K0[:3] + 0.5*K3)
...
# for i in range(N):
#     if  i < 3:
#         temp = activate(data[-1],np.deg2rad(0),13.9,w_flag =True)
#     elif i > 3:
#         temp = activate(data[-1],np.deg2rad(35),13.9,w_flag =True)
#     data.append(temp)
#     x1.append(temp[3]/7)
#     y1.append(temp[4]/7)
...
#     if  j < 3:
#         temp1 = activate(data1[-1],np.deg2rad(0),13.9,w_flag =True)
#     elif j > 3:
#         temp1 = activate(data1[-1],-np.deg2rad(35),13.9,w_flag =True)
