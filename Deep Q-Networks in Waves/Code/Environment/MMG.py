...
###############################################
try:
    X_wave = np.array(scipy.io.loadmat(os.getcwd()+"\\"+"Environment"+"\\"+"Wave_Data"+"\\" +"DX.mat")['DX'])
    Y_wave = np.array(scipy.io.loadmat(os.getcwd()+"\\"+"Environment"+"\\"+"Wave_Data"+"\\" +"DY.mat")['DY'])
    N_wave = np.array(scipy.io.loadmat(os.getcwd()+"\\"+"Environment"+"\\"+"Wave_Data"+"\\" +"DN.mat")['DN'])
except:
    X_wave = np.array(scipy.io.loadmat(os.getcwd()+"\\"+"Wave_Data"+"\\" +"DX.mat")['DX'])
    Y_wave = np.array(scipy.io.loadmat(os.getcwd()+"\\"+"Wave_Data"+"\\" +"DY.mat")['DY'])
    N_wave = np.array(scipy.io.loadmat(os.getcwd()+"\\"+"Wave_Data"+"\\" +"DN.mat")['DN'])
    
###############################################
############### MMG Model Group ###############
###############################################

def MMG_Time_Derivative(u,v,r,x,y,psi,delta,w_flag,rps = 12.05):
    """
    ----------
    ...
    # print(F_propellar,"propellar here")
    #####################################################
    ...
    Amplitude          = 3.2 #3.2/45.7142
    wave_frequency     = 0.4390*np.sqrt(7/320)  #*(np.sqrt(Lamda))*0.8
    heading_angle      = -np.pi
    
    A1  =  np.real(X_wave[::,1:]) * (Amplitude**2)
    A2  = -np.real(Y_wave[::,1:]) * (Amplitude**2)
    A3  = -np.real(N_wave[::,1:]) * (Amplitude**2)

    wave_drift     = np.real(2*np.pi*X_wave[::,0])
    hangle_drift   = -np.arange(-np.pi,np.pi+0.01,np.deg2rad(15))
    hangle_desire  = heading_angle - LOS.los_mmg_normalizer(psi) #yaw angle
    ...
    X_Drift =  FS_force  * fA1(hangle_desire,wave_frequency)
    Y_Drift =  FS_force  * fA2(hangle_desire,wave_frequency)
    N_Drift =  FS_moment * fA3(hangle_desire,wave_frequency)
        
    F_Drift = np.array([[X_Drift[0]],[Y_Drift[0]],[N_Drift[0]]])

    # print(F_Drift,"drift is here")
    #####################################################
...
def activate(ip,delta,rps,w_flag = False):
    ...
    ###############################
      ...
      K1_ = MMG_Time_Derivative(u,v,r,x,y,psi,delta,rps,w_flag)
      K1  = h*K1_[:3] 
      k1  = h*K0[:3]
...
############# To check ##############################
#####################################################
### when you evaluate, please change the wave folder#
#####################################################
# ip = [1.149,0,0,0,0,0,0,0]
# print(ip)
# data= list()
# data1 = list()
# data.append(ip)
...
#     temp = activate(data[-1],-np.deg2rad(35),True)
#     data.append(temp)
#     x1.append(temp[3])
#     y1.append(temp[4])
