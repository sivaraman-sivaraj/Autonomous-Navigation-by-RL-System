...
def activate(ip,RuPos,delta,rps,WHA,w_flag): 
    ...
    Parameters
    ----------
    ip         : (u,v,r,x,y,psi,HE,CTE)
    RuPos      : Rudder Position
    delta      : commanded rudder angle
    rps        : revolution per second
    WHA        : Wave Head Angle
    w_flag     : True or False for inclusion of wave force

    Returns
    -------
    next state, rudder position

    """
    h = 1 # time step as increment
    u,v,r,x,y,psi = ip[0],ip[1],ip[2],ip[3],ip[4],ip[5]
    ######### Rudder Command Assertion ######
    if abs((RuPos - delta)/h) > (np.deg2rad(10)):
        delta = RuPos + (np.sign(delta)*np.deg2rad(10)*h)
    else :
        delta = delta
    ...
    return nxt_state.tolist(), RuPos_new
...
# cw,ccw = [],[]
# for i in range(N):
#     temp, RuPos_New = activate(data[-1],RuPos,np.deg2rad(40),rps,WHA,w_flag =FF)
#     RuPos           = RuPos_New
#     data.append(temp)
#     x1.append(temp[3]/7)
#     y1.append(temp[4]/7)
...
# RuPos  = 0.0
# for j in range(N):
#     temp1,RuPos_New = activate(data1[-1],RuPos,-np.deg2rad(40),rps,WHA,w_flag =FF)
#     RuPos           = RuPos_New
#     data1.append(temp1)
#     x2.append(temp1[3]/7)
#     y2.append(temp1[4]/7)
