...
class load(Env): # NavigationRL-v0
     ...
     def reset(self):
        ...
        self.RusPos                           = 0.0
        return self.current_state,self.H,self.RusPos
    
     def step(self,C):
         self.action,self.H,self.WHA, self.RusPos = C[0],C[1],C[2],C[3]
         self.goal                                = self.H[2]
         self.ip                                  = self.current_state.clone().detach() #ip,RuPos,delta,rps,WHA,w_flag
         self.op,self.RuPos_new                   = MMG.activate(self.ip,self.RusPos,np.deg2rad(self.actions_set[str(self.action)]),self.rps,self.WHA,self.W_Flag)
         ...
         return self.current_state, [self.reward_a,self.HE,self.RuPos_new], self.done,self.H
