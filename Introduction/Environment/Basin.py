...
class Surrounding(Env): #basinGrid-s0
     ...
     def __init__(self,R,prp,grid_size,land,green_water,wave= False):
        ...
        self.actions = np.array([0,1,2]) #set of actions
        self.act0 = self.actions[0] # right
        self.act1 = self.actions[2] # left
        self.act2 = self.actions[1] # up
        self.rewards = self.R
        ...
        right_land = [] # (0,0) to (land,0) in width and (grid_size) in full size
        for i in range(land):
            for j in range(grid_size):
                right_land.append((0+i,0+j))
        self.right_land = np.array(right_land)
        
        left_land = [] # (grid_size-land,0) to (grid_size,0) in width and (grid_size) in full size
        for i in range(land):
            for j in range(grid_size):
                left_land.append((grid_size-land+i,0+j))
        self.left_land = np.array(left_land)
        
        bottom_land = [] # (land,0) to (grid_size-land,0) in width and (land) in height
        for i in range(grid_size-(2*land)):
            for j in range(land):
                bottom_land.append((land+i,0+j))
        self.bottom_land = np.array(bottom_land)
        
        top_land = [] # (land,grid_size-land) to (grid_size-land,grid_size-land) in width and (land) in height
        for i in range(grid_size-(2*land)):
            for j in range(land):
                top_land.append((land+i,grid_size-land+j))
        self.top_land = np.array(top_land)
        
        ###############################
        ######## Green Water ##########
        ###############################
        right_green_water = [] #(grid_size-land-green_water,land) to (grid_size-land,land) in width and (grid_size-(2*land)) in height
        for i in range(green_water):
            for j in range(grid_size-(2*land)):
                right_green_water.append((grid_size-land-green_water+i,land+j))
        self.right_green_water = np.array(right_green_water)
        
        left_green_water = [] #(land,land) to (land+green_water,land) in width and (grid_size-(2*land)) in height
        for i in range(green_water):
            for j in range(grid_size-(2*land)):
                left_green_water.append((land+i,land+j))
        self.left_green_water = np.array(left_green_water)
        
        bottom_green_water = [] #(land+green_water,land) to (grid_size-land-green_water,land) in width and (green_water) in height
        for i in range(grid_size-(2*land)-(2*green_water)):
            for j in range(green_water):
                bottom_green_water.append((land+green_water+i,land+j))
        self.bottom_green_water = np.array(bottom_green_water)

        top_green_water = [] #(land+green_water,grid_size-land-green_water) to (grid_size-land-green_water,grid_size-land-green_water) in width and (green_water) in height
        for i in range(grid_size-(2*land)-(2*green_water)):
            for j in range(green_water):
                top_green_water.append((land+green_water+i,grid_size-land-green_water+j))
        self.top_green_water = np.array(top_green_water)
     ...
     def reset(self):
        self.current_state = [self.st_x,self.st_y]
        self.done = False
     ...
     def step(self, action):
        ...
        if(action==0): #right
            self.next_state[0] = min(self.current_state[0]+1, self.grid_size-self.land-self.green_water)
            self.next_state[1] = min(self.current_state[1]+0, self.grid_size-self.land-self.green_water)
        elif(action==2): #left
            self.next_state[0] = max(self.current_state[0]-1, self.land+self.green_water+1)
            self.next_state[1] = min(self.current_state[1]+0, self.grid_size-self.land-self.green_water)
        elif(action==1): #up
            self.next_state[0] = min(self.current_state[0]+0, self.grid_size-self.land-self.green_water)
            self.next_state[1] = min(self.current_state[1]+1, self.grid_size-self.land-self.green_water) 
        ...
        '''
        if self.next_state[0] == self.goals[0] and self.next_state[1] == self.goals[1] :
            self.done = True
        if reward == (-100):
            self._ = 'Not in water'
        if reward == 100:
        ...
        return self.current_state, reward, self.done,self._
     ...
     def render(self,tp,mode='human'):
        ...
        #Green water surface_left
            ...
            ############### Ship Rendering ####################
            ###################################################            
            ship = rendering.FilledPolygon(import_polygon(self.sr))
         ...
         shipx = self.current_state[0]/self.sr
         shipy = self.current_state[1]/self.sr
